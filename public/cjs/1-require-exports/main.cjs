...
CJS동작 원리
Resolutuion(모듈경로확인) - Loading(모듈로드) - Wrapping(코드래핑)
...
-> 모듈 타입을 확인하고 파싱 및 로드하는 과정 
  ...
  -> .node (C++바이너리코드)
  -> cannot find module
-> 코드 래핑 : 파일이름이 다른데 변수이름이 같아도 에러가 발생하지 않는다. 
...
const newMember = memberCreate.createMember("김유신","kim@gmail.com","마포구 공덕동")
